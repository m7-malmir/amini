        ...
        try {
            ...
            foreach ($newFiles as $file) {
                if (isset($file['tmp_name']) && $file['error'] === UPLOAD_ERR_OK) {
                    // بررسی حجم فایل
                    if ($file['size'] > $this->maxFileSize) {
                        $_SESSION['error'] = 'حجم عکس بالاس-ریپ';
                        header("location: ../manage/add-product");
                            exit();
                    }
                    ...
                    if (!in_array(mime_content_type($file['tmp_name']), $this->allowedTypes)) {
                        $_SESSION['error'] = 'نوع فایل درست نیست-ریپ';
                        header("location: ../manage/add-product");
                            exit();
                    }
                    ...
                    //     $_SESSION['error'] = 'مسیر غیرمجاز-ریپ';
                    //     header("location: ../manage/add-product");
                    //         exit();
                    // }
                    ...
                    if (move_uploaded_file($file['tmp_name'], $destination)) {
                        $uploadedPaths[] = '../upload/'.$uniqueName; // مسیر جدید ذخیره‌شده
                    } else {
                        $_SESSION['error'] = 'مسیر فایل جدید-ریپ';
                        header("location: ../manage/add-product");
                            exit();
                    }
                } else {
